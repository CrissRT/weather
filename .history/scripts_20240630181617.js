        ...
        fetch(fetch_link)
            ...
            .then(data => {
                coordinates = (data.length !== 0) 
                    ? data.slice(0,4).map(location =>{lat: location.lat; lon: location.lon, name = location.display_name})
                    : null;
                    
                search_output.innerHTML = coordinates.map(location => location.name).join(<be></be>)
            })
            .catch(error => {
