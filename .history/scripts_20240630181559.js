        ...
        fetch(fetch_link)
            ...
            .then(data => {
                coordinates = (data.length !== 0) 
                    ? data.slice(0,4).map(location =>{lat: location.lat; lon: location.lon, location_name = location.display_name})
                    : null;
                    
                search_output.innerHTML = coordinates.map(location => location.d)
            })
            .catch(error => {
