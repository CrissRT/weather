        ...
        fetch(fetch_link)
            ...
            .then(data => {
                coordinates = (data.length !== 0) 
                    ? data.slice(0,4).map(location =>{lat: location.lat; lon: location.lon, location = location.display_name})
                    : null;
                    
                search_output.innerHTML = coordinates.map(location => location.display_name)
            })
            .catch(error => {
